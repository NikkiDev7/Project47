...
var timer = 5;
var gameState = "start";
var y = [230, 205, 180, 155, 130, 105];
var x = [15,40,65,90,115,140,165,190,215,240,265,290,315,340,365,390,415,440,465,490,515,540,565,590,615,640,665,690,715,740,765,790,815,840];
...
function draw() {
    ...
    if (gameState === "play") {
        ...
        if(frameCount%20 == 0 ){
            spikes = createSprite(random(x), random(y))
            spikes.addImage('SpikesImage', spikesImg);
            spikes.scale = 1.5;
            spikesGroup.add(spikes);

            zombie.depth=spikes.depth;
            zombie.depth +=1;
    
            wizard.depth=spikes.depth;
            wizard.depth +=1;
    
            if(wizard.isTouching(spikesGroup)) {
                reset();
            }
        }
